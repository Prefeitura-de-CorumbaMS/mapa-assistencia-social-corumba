...
// ============================================================================
// UNIDADE DE ASSISTÊNCIA SOCIAL ROUTES
// ============================================================================

/**
 * GET /api/unidades
 * Lista todas as unidades (público)
 */
router.get('/', asyncHandler(async (req, res) => {
  ...
  const [unidades, total] = await Promise.all([
    prisma.pROD_UnidadeAssistenciaSocial.findMany({
      where,
      skip,
      take: parseInt(limit),
      include: {
    ...
    prisma.pROD_UnidadeAssistenciaSocial.count({ where }),
  ]);
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  const unidade = await prisma.pROD_UnidadeAssistenciaSocial.findUnique({
...
router.post('/', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  ...
  // Preparar conexão com ícone (se fornecido)
  let iconeConnect = undefined;
  if (id_icone) {
    iconeConnect = { connect: { id: id_icone } };
  }

  // Criar unidade
  const unidade = await prisma.pROD_UnidadeAssistenciaSocial.create({
    data: {
      nome,
      nome_fantasia,
      razao_social,
  ...
  if (categorias.length > 0) {
    await prisma.junction_UnidadeAssistenciaSocial_Categoria.createMany({
  ...
  auditLog('CREATE', 'PROD_UnidadeAssistenciaSocial', unidade.id, req.user.id, req.user.role);
...
router.put('/:id', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const existingUnidade = await prisma.pROD_UnidadeAssistenciaSocial.findUnique({
    where: { id: parseInt(id) },
  });
  ...
  // Atualizar relacionamento com ícone (usar connect/disconnect ao invés de id_icone direto)
  if (id_icone !== undefined) {
    if (id_icone === null) {
      updateData.icone = { disconnect: true };
    } else {
      updateData.icone = { connect: { id: id_icone } };
    }
  }
  ...
  // Atualizar unidade
  const unidade = await prisma.pROD_UnidadeAssistenciaSocial.update({
    where: { id: parseInt(id) },
    data: updateData,
  });
  ...
  if (categorias !== undefined) {
    ...
    await prisma.junction_UnidadeAssistenciaSocial_Categoria.deleteMany({
  ...
  auditLog('UPDATE', 'PROD_UnidadeAssistenciaSocial', unidade.id, req.user.id, req.user.role, { updateData });
...
router.delete('/:id', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  ...
  await prisma.pROD_UnidadeAssistenciaSocial.delete({
    where: { id: parseInt(id) },
  });

  auditLog('DELETE', 'PROD_UnidadeAssistenciaSocial', parseInt(id), req.user.id, req.user.role);

  logger.info('Unidade deletada', {
    user_id: req.user.id,
 ...
 * Lista redes sociais de uma unidade
 */
router.get('/:id/redes-sociais', asyncHandler(async (req, res) => {
  const { id } = req.params;

  const redesSociais = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.findMany({
    where: { id_unidade: parseInt(id) },
  });
 ...
 * Adiciona rede social a uma unidade
 */
router.post('/:id/redes-sociais', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const count = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.count({
    where: { id_unidade: parseInt(id) },
  });
  ...
  const existing = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.findFirst({
    where: {
      id_unidade: parseInt(id),
      nome_rede,
    },
  ...
  const redeSocial = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.create({
    data: {
      id_unidade: parseInt(id),
      nome_rede,
      url_perfil,
 ...
 * Atualiza rede social de uma unidade
 */
router.put('/:id/redes-sociais/:redeId', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  const { id, redeId } = req.params;
  const { nome_rede, url_perfil } = req.body;

  const redeSocial = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.findFirst({
    where: {
      id: parseInt(redeId),
      id_unidade: parseInt(id),
    },
  ...
  const updated = await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.update({
 ...
 * Remove rede social de uma unidade
 */
router.delete('/:id/redes-sociais/:redeId', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  ...
  await prisma.pROD_UnidadeAssistenciaSocial_RedeSocial.delete({
    where: { id: parseInt(redeId) },
  });
...
/**
 * GET /api/unidades/:id/setores
 * Lista setores de uma unidade de assistência social
 */
router.get('/:id/setores', asyncHandler(async (req, res) => {
  const { id } = req.params;

  // Buscar setores através da junction table
  ...
  const setores = junctions.map(j => j.setor);
...
/**
 * POST /api/unidades/:id/setores
 * Adiciona setor a uma unidade de assistência social
 */
router.post('/:id/setores', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { nome, gestor, numero, whatsapp, email } = req.body;
  ...
  // Criar setor
  const setor = await prisma.pROD_Setor.create({
    data: {
      nome,
      gestor,
      numero,
...
/**
 * PUT /api/unidades/:id/setores/:setorId
 * Atualiza setor de uma unidade
 */
router.put('/:id/setores/:setorId', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  const { id, setorId } = req.params;
  const { nome, gestor, numero, whatsapp, email } = req.body;

  // Verificar se o setor pertence à unidade
  const junction = await prisma.junction_UnidadeSetor.findFirst({
    where: {
      id_unidade: parseInt(id),
      id_setor: parseInt(setorId),
    },
  });
  ...
  if (!junction) {
    return res.status(404).json({
      success: false,
      error: 'Setor não encontrado para esta unidade',
    });
  }

  const setor = await prisma.pROD_Setor.update({
    where: { id: parseInt(setorId) },
    data: {
      ...(nome && { nome }),
      ...(gestor !== undefined && { gestor }),
...
/**
 * DELETE /api/unidades/:id/setores/:setorId
 * Remove setor de uma unidade
 */
router.delete('/:id/setores/:setorId', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  const { id, setorId } = req.params;
...
router.get('/stats/last-update', asyncHandler(async (req, res) => {
  const lastUnidade = await prisma.pROD_UnidadeAssistenciaSocial.findFirst({
