...
import { useGetUnidadesQuery, useGetLastUpdateQuery, useGetIconesQuery, useGetCategoriasGroupedQuery, useGetSetoresQuery } from '../store/slices/apiSlice'
...
export default function MapPage() {
  ...
  // Buscar setores da unidade selecionada
  const { data: setoresData } = useGetSetoresQuery(selectedUnidade?.id, {
    skip: !selectedUnidade?.id
  })
  ...
  const filteredUnidades = useMemo(() => {
    ...
    if (selectedIconUrl) {
      ...
      filtered = filtered.filter(unidade => {
        // Usar icone.url (novo campo) ou icone_url (deprecated) como fallback
  ...
  return (
    <>
            ...
            {selectedUnidade ? (
              <div style={{ height: '100%' }}>
                ...
                <div style={{ padding: '24px' }}>
                  ...
                  {/* Secretária e Secretária Adjunta */}
                  {(selectedUnidade.secretaria || selectedUnidade.secretaria_adjunta) && (
                    <div style={{
                      ...
                      {selectedUnidade.secretaria && (
                        <div style={{
                          marginBottom: selectedUnidade.secretaria_adjunta ? '12px' : '0',
                        }}>
                          <div style={{
                            fontSize: '12px',
                      ...
                      {selectedUnidade.secretaria_adjunta && (
                        <div>
                          <div style={{
                            fontSize: '12px',
                            color: 'rgba(255, 255, 255, 0.9)',
                  ...
                  {/* Serviços */}
                  {setoresData?.data && setoresData.data.length > 0 && (
                    <div style={{ marginBottom: '24px' }}>
                      <Button
                        type="primary"
                        size="large"
                        block
                        icon={<TeamOutlined />}
                        onClick={() => setSetoresModalVisible(true)}
                        style={{
                          background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                          borderColor: '#667eea',
                          height: '48px',
              ...
              <div style={{ padding: '24px' }}>
                <div style={{ textAlign: 'center', paddingTop: '40px' }}>
                  ...
                  <h2 style={{ fontSize: '22px', fontWeight: 'bold', color: '#333' }}>
                    Bem-vindo ao Mapa da Assistência Social
                  </h2>
                  <p style={{ fontSize: '16px', color: '#666', lineHeight: 1.6, marginTop: '16px', marginBottom: '24px' }}>
                    Encontre serviços, gerências e programas da Assistência Social em Corumbá.
                    <br />
                    Clique em um ponto no mapa para ver informações e contatos.
                  </p>
                  ...
                  >
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <Input
                        placeholder="Digite para buscar serviço, bairro ou categoria (ex.: CRAS, abrigamento, auxílio)"
                      ...
                      {/* Se selecionou categoria principal, abrir seleção de subcategorias (apenas se houver subcategorias) */}
                      {selectedCategoriaNome && (
                        (() => {
                          const group = categoriasGrouped.find(g => g.nome === selectedCategoriaNome) || { subcategorias: [] }
                          // Só mostrar select se houver subcategorias
                          if (group.subcategorias.length === 0) return null
                      ...
                      {/* Select direto para Buscar por Unidade */}
                      {!searchText && (
                        <>
                          <Select
                            ...
                            onChange={(value) => {
                              ...
                              // Limpar busca por texto mas manter filtros de categoria
                              setSearchText('')
                  ...
                  {/* Rodapé com informações da Prefeitura */}
                  <div style={{
                    marginTop: '24px',
                    padding: '12px',
                    fontSize: '12px',
        ...
        <div style={{ 
          ...
          >
            ...
            {filteredUnidades.map((unidade) => {
              ...
              // Usar icone.url (novo campo) ou icone_url (deprecated) como fallback
              let customIcon = null;
              const iconUrl = unidade.icone?.url || unidade.icone_url
      ...
      {/* Modal de Setores */}
      <SetoresModal
        visible={setoresModalVisible}
        onClose={() => setSetoresModalVisible(false)}
        unidadeId={selectedUnidade?.id}
        unidadeNome={selectedUnidade?.nome}
      />
    </>
