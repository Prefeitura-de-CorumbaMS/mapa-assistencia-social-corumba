...
export const apiSlice = createApi({
  ...
  tagTypes: ['Unidades', 'Categorias', 'Medicos', 'Especialidades', 'Staging', 'Users', 'Audit', 'ETL', 'Mapeamentos', 'Bairros', 'OfertasEnsino', 'Icones', 'Setores'],
  ...
  endpoints: (builder) => ({
    ...
    // Setores
    getSetores: builder.query({
      query: (unidadeId) => `/unidades/${unidadeId}/setores`,
      providesTags: ['Setores'],
      keepUnusedDataFor: 300,
    }),

    createSetor: builder.mutation({
      query: ({ id, ...data }) => ({
        url: `/unidades/${id}/setores`,
        method: 'POST',
        body: data,
      }),
      invalidatesTags: ['Setores', 'Unidades'],
    }),

    updateSetor: builder.mutation({
      query: ({ id, setorId, ...data }) => ({
        url: `/unidades/${id}/setores/${setorId}`,
        method: 'PUT',
        body: data,
      }),
      invalidatesTags: ['Setores', 'Unidades'],
